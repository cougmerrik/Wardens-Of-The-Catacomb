...
- The shop pool includes tactical utility consumables such as Lantern Fuel, Darkvision Potion, Holy Candle, and multiplayer-only revive consumables. Lantern Fuel refills the purchaser's lantern by 20%, Darkvision Potion privately gives the consumer 10 tiles of dark sight for 30 seconds, Holy Candle drops a 3-tile holy light for 10 seconds that heals players inside for 5% max health each second, Phoenix Draught revives one random dead ally at the user's position with 40% HP, and the once-per-run Flame of the Fallen creates a 20s pyre that revives all dead allies at 50% HP if nearby enemy kills fill its soul meter. Spike Growth is charge-based, retaliating for the next 25 hits instead of expiring by time.
