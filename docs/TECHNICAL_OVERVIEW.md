...
- Ghost enemies use indexed Blender-authored sheets from `assets/images/enemies/ghost_family/`. `src/rendering/ghostSpriteSheet.js` selects 128 px cells for three stable variants, pale-white and pale-green palette presentations, six actions, and eight runtime directions, then draws them at 44 px with nearest-neighbor sampling. Their solid collision core is 16 px so transparent shroud and glow pixels do not create an unforgiving body hitbox. Shadow, semi-translucent color, and tight glow remain separate sprite passes; ghost glow does not add a world-light source. Controlled undead retain their owner-colored ground accent without replacing the pale spectral palette. Spawn and network snapshots preserve `ghostVariant`, `ghostPalette`, a stable randomized loop phase, facing, action, action start, hurt, siphon, and dive state for local/multiplayer presentation parity without synchronized looping. Death clamps to its final sheet frame for a 30-second non-blocking remnant and does not use the procedural floor-mist fallback. If a live image is unavailable, the renderer falls back to the legacy procedural ghost.
...
- Focused validation:
  ...
  - `npm run validate:ghost-sprites` validates all 72 permanent ghost sheets (including the retained silhouette validation pass), sheet dimensions, direction order, action/death timing, spawn variability, and serialized ghost presentation fields.
