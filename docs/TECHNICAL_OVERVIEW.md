...
- Floor-boss state in `src/game/runtimeFloorBossMethods.js` is boss-type aware and owns post-defeat cleanup state. Boss defeat stamps a `10s` same-floor spawn suppression window, queues a one-shot hidden-hostile cleanup using the union of living player viewports, then lets the spawn interval helper run at half rate for the rest of that floor. Combat resolution suppresses non-boss post-boss loot without treating silently removed enemies as deaths.
