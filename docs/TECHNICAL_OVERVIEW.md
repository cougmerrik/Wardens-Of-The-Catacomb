...
- Consumables can modify lighting: Lantern Fuel calls the centralized lantern fuel helper, Darkvision Potion contributes a private 10-tile local player sight radius while active, and Holy Candle creates a timed `holyCandle` light source that heals living players in its radius.
